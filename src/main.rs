...
// Re-export core types for convenience within this file.
...
mod tests {
    ...
    /// Evaluate Forth code and return the captured output.
    ...
    /// Evaluate and return the top of the data stack.
    ...
    fn test_recursion_factorial() {
        let mut vm = test_vm();
        eval(&mut vm, ": FACT DUP 1 > IF DUP 1 - RECURSE * ELSE DROP 1 THEN ;");
    ...
    fn test_begin_while_repeat() {
        let mut vm = test_vm();
        eval(&mut vm, ": WH 5 BEGIN DUP 0 > WHILE DUP . 1 - REPEAT DROP ;");
    ...
    fn test_nested_if() {
        let mut vm = test_vm();
        eval(&mut vm, ": SIGN DUP 0 > IF 1 ELSE DUP 0 < IF -1 ELSE 0 THEN THEN NIP ;");
    ...
    fn test_prelude_loads() {
        ...
        let names: Vec<&str> = vec!["NIP", "TUCK", "2DUP", "2DROP", "ABS", "MIN", "MAX",
                                     "NEGATE", "1+", "1-", "TRUE", "FALSE"];
        for name in names {
            assert!(vm.find_word(name).is_some(), "prelude word '{}' not found", name);
        }
    }
    ...
    fn test_snapshot_roundtrip() {
        ...
        let data = persist::serialize_snapshot(&snap);
        let restored = persist::deserialize_snapshot(&data).unwrap();
    ...
    fn test_package_build_unpack() {
        ...
        let pkg = spawn::build_package(&state).unwrap();
        let (binary, state_out, prelude) = spawn::unpack_package(&pkg).unwrap();
    ...
    fn test_package_invalid_magic() {
        ...
        assert!(spawn::unpack_package(&bad).is_err());
    }
    ...
    fn test_serialize_state_roundtrip() {
        ...
        let goals = goals::GoalRegistry::empty();
        let data = mesh::serialize_state(&vm.dictionary, &vm.memory, vm.here, Some(&goals));
        let (dict, mem, here) = mesh::deserialize_state(&data).unwrap();
