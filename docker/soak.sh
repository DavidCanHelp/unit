...
report)
    ...
    END {
        ...
        stale=0; stale_units=0; births=0
        for (id in last_t) {
            ...
            units += last_u[id]; out += last_o[id]; inn += last_n[id]; deaths += last_d[id]; births += last_b[id]
        ...
        printf "(soak-colony :ticks %d :units %d :migrations-out %d :migrations-in %d :deaths %d :births %d :stale-nodes %d :stale-units %d :kinds-gained %d :fit-gained %d :copies-gained %d :verdict %s)\n", \
            ticks, units, out, inn, deaths, births, stale, stale_units, kg, fg, cg, verdict
        # Event-consistent accounting:
        #   units == expected + landed - released - starved + born
        ...
        printf "(soak-conservation :units %d :expected 900 :out %d :in %d :deaths %d :births %d :balanced %s)\n", \
            units, out, inn, deaths, births, (stale > 0 ? "stale-ledger" : (units == 900 + inn - out - deaths + births ? "yes" : "VIOLATION"))
    }' "$LOGS"/s1.log "$LOGS"/s2.log "$LOGS"/s3.log
    ;;
down)
