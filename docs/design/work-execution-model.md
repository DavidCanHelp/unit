...
- **Observability over caps.** Every slot counts its reassignments (gossip-death or timeout), surfaced by `RECRUITS`, so re-recruit cycling between wedged peers is visible on hardware. No reassignment cap in v1 — each cycle costs one message per timeout period, and a cap would be a guess; add one only if cycling is actually observed. *(Superseded: unbounded cycling WAS observed — since v0.35, `MAX_SLOT_ATTEMPTS` = 5 caps combined reassignments and fail-closed resets, after which the slot is abandoned with an error that self-reports up the tree; total per-slot wait ≤ `RECRUIT_TIMEOUT` × 6.)* (Extended after the 2026-06-10 hardware session: fail-closed deadline resets are counted and surfaced too — without that, "timeout pass failing closed every 60s" and "timeout pass not firing at all" rendered identically as a bare pending slot.)
...
Observed on hardware: first-sufficient placement keeps the largest part local whenever it fits the local tally, exporting only the trivial parts — a node at 92% headroom kept a 600MB subtree on a 456MB box while shipping its 50MB leaf to a peer. Most-headroom-among-abundant placement (Tier 1 in code) is never consulted when the local fit check says "I fit", so part size and destination capacity are never matched against each other.
