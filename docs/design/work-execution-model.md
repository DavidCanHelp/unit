...
- **Observability over caps.** Every slot counts its reassignments (gossip-death or timeout), surfaced by `RECRUITS`, so re-recruit cycling between wedged peers is visible on hardware. No reassignment cap in v1 — each cycle costs one message per timeout period, and a cap would be a guess; add one only if cycling is actually observed.
